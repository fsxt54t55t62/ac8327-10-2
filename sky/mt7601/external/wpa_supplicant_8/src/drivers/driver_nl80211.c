...
extern void wpa_supplicant_stage_scan_reschedule(void *ctx);
#endif
				      ...
				      const u8 *key, size_t key_len)
{
	...
	if (nla_put_u8(msg, NL80211_ATTR_KEY_IDX, key_idx))
		goto fail;
#if CONFIG_MEDIATEK
	/* increase the reference count of msg,
	so it will be not freed in the followed calling of send_and_recv_msgs */
	nlmsg_get(msg);
	/* the last param for send_and_recv_msgs should not be -1, otherwise, data for msg will be cleared */
	...
	if ((ret == -ENOENT || ret == -ENOLINK) && alg == WPA_ALG_NONE)
		ret = 0;
#if CONFIG_MEDIATEK
	/* if error code is -ENOLINK, it may be due to too fast to done 4-way handshake.
		let's try again to set key */
	if (ret == -ENOLINK) {
		wpa_printf(MSG_INFO, "nl80211: failed to set key due to No link, wait 0.5s and try again");
		os_sleep(0, 500000); /* wait cfg80211 work queue 0.5s to be ready */
		ret = send_and_recv_msgs(drv, msg, NULL, key ? (void *) -1 : NULL);
	} else {
		nl80211_nlmsg_clear(msg);
		nlmsg_free(msg);
...
#ifdef ANDROID_P2P
/*
* [ALPS00326459] [6577JB][CTS Verifier 4.1_r1][MT6577_PHONE] [changelist 983747]
* GROUP JOIN 2 cases fail in Wi-Fi Direct Test
* MTK driver has not implement P2P ps function, the 3rd lib also returns -1.
*/
